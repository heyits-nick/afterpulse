...
export default function Home() {
  ...
  const hrPath = linePath(
    visibleReplay.map((point) => point.hr),
    65,
    130,
  );
  const hrvPath = linePath(
  ...
  const statusMessage =
    phase === 'replaying'
      ...
      : phase === 'ready'
        ? 'Recovery detected · consent confirmed'
        : phase === 'completed'
          ? 'Clinician record ready'
          : bridgeMessage;
  ...
  useEffect(() => {
    ...
    async function pollBridge() {
      const bridgeEpoch = bridgeEpochRef.current;
      try {
        ...
        if (!mounted || bridgeEpoch !== bridgeEpochRef.current) return;
  ...
  async function startCall() {
    ...
    try {
      ...
      setError(
        callError instanceof Error ? callError.message : 'Unable to start call',
      );
      setPhase('error');
    }
  }
      ...
      <header className="sticky top-0 z-40 border-b border-white/70 bg-card/70 shadow-[0_1px_0_rgba(15,76,61,0.04)] backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1480px] items-center justify-between gap-4 px-5 py-4 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Post-event care, while the memory is fresh
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Badge
              variant="outline"
              className="hidden border-emerald-200 bg-emerald-50 text-emerald-800 sm:inline-flex"
            >
              <ShieldCheck data-icon="inline-start" />
              Local demo · Not diagnostic
            </Badge>
            <Badge
              variant="outline"
              className={
      ...
      <section className="mx-auto max-w-[1480px] px-5 py-6 lg:px-8 lg:py-8">
        <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          ...
          <div className="flex items-center gap-3">
            <p
              aria-live="polite"
              className="max-w-sm text-right text-sm text-muted-foreground"
            >
              {statusMessage}
            </p>
            {phase !== 'idle' && (
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[1.08fr_0.92fr]">
          <Card className="relative overflow-hidden border border-white/80 bg-card/80 shadow-[0_24px_80px_-48px_rgba(11,67,54,0.65)] ring-1 ring-emerald-950/[0.03] backdrop-blur-xl transition-all duration-300 before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-linear-to-r before:from-transparent before:via-emerald-300/70 before:to-transparent hover:-translate-y-0.5 hover:shadow-[0_28px_90px_-46px_rgba(11,67,54,0.75)]">
            <CardHeader className="border-b border-border/70 pb-4">
              ...
              <CardDescription>
                Galaxy Watch4 · Deterministic synthetic replay
              </CardDescription>
              <CardAction>
            ...
            <CardContent className="space-y-5">
              <div className="grid grid-cols-3 gap-3">
                <Metric
                  label="Heart rate"
                  value={String(reading.hr)}
                  unit="BPM"
                  icon={<HeartPulse />}
                  emphasis={reading.hr >= 100}
                />
                <Metric
                  label="HRV"
                  value={String(reading.hrv)}
                  unit="MS"
                  icon={<Activity />}
                  emphasis={reading.hrv <= 28}
                />
                <Metric
                  label="Motion"
                  value="Low"
              ...
              <div className="rounded-2xl border border-white/80 bg-linear-to-br from-[#f8fbf9] via-white to-emerald-50/50 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.9)]">
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Compressed 12-minute episode replay
                    </p>
                  </div>
                  <div className="flex gap-3 text-[11px] text-muted-foreground">
                    <span className="flex items-center gap-1.5">
                      <i className="size-2 rounded-full bg-[#e66a58]" /> Heart
                      rate
                    </span>
                    <span className="flex items-center gap-1.5">
                      <i className="size-2 rounded-full bg-[#14866d]" /> HRV
                    </span>
                  </div>
                </div>
                <svg
                ...
                >
                  ...
                  <text x="12" y="218" fill="#73827c" fontSize="10">
                    12 min ago
                  </text>
                  <text x="670" y="218" fill="#73827c" fontSize="10">
                    now
                  </text>
                </svg>
              </div>
...
function PatientAction({
      ...
      <div className="flex flex-col justify-between gap-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 sm:flex-row sm:items-center">
        <div>
          <p className="flex items-center gap-2 font-medium text-emerald-950">
            <Check className="size-4" /> Recovery detected
          </p>
          <p className="mt-1 text-sm text-emerald-800/75">
            {bridgeOnline
              ? 'The patient has opted in to a one-minute post-event check-in.'
              : 'The Guava bridge is offline. Continue with a clearly labeled sample result.'}
          </p>
        </div>
        <Button
          size="lg"
        ...
        >
          ...
          {bridgeOnline ? 'Call my iPhone' : 'Show sample clinician record'}
        </Button>
      </div>
    );
  }
  ...
  if (phase === 'calling' || phase === 'in_call' || phase === 'processing') {
    const copy =
      phase === 'calling'
        ? [
            'Calling the consented iPhone',
            'The real Guava call should ring in a few seconds.',
          ]
        : phase === 'in_call'
          ? [
              'Check-in in progress',
              'Maya is collecting the six clinician-review fields.',
            ]
          : [
              'Structuring clinician record',
              'The call is complete; responses are being saved.',
            ];
    return (
      ...
      <div className="flex flex-col justify-between gap-4 rounded-2xl border border-red-200 bg-red-50 p-5 sm:flex-row sm:items-center">
        <div>
          <p className="flex items-center gap-2 font-medium text-red-900">
            <AlertCircle className="size-4" /> Check-in could not start
          </p>
          <p className="mt-1 max-w-xl text-sm text-red-800/75">
            {error || 'Check the local Guava bridge and try again.'}
          </p>
        </div>
  ...
  if (phase === 'completed') {
    return (
      <div className="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-emerald-950">
        ...
        <div>
          <p className="font-medium">Care loop complete</p>
          <p className="text-sm text-emerald-800/70">
            The structured record is ready for clinician review.
          </p>
        </div>
      </div>
    );
    ...
    <div className="flex flex-col justify-between gap-4 rounded-2xl bg-[#112a24] p-5 text-white sm:flex-row sm:items-center">
      <div>
        <p className="font-medium">
          {phase === 'replaying'
            ? 'Replaying physiological episode'
            : 'Ready to demonstrate the care loop'}
        </p>
        <p className="mt-1 text-sm text-emerald-100/75">
          {phase === 'replaying'
            ? 'Applying the recovery-state heuristic…'
            : 'Replay 12 minutes of wearable data in under 10 seconds.'}
        </p>
      </div>
      <Button
        size="lg"
      ...
      >
        ...
        {phase === 'replaying' ? 'Replay running' : 'Replay wearable episode'}
      </Button>
    </div>
  );
}
  ...
  return (
    <Card className="relative overflow-hidden border border-white/80 bg-card/80 shadow-[0_24px_80px_-48px_rgba(11,67,54,0.65)] ring-1 ring-emerald-950/[0.03] backdrop-blur-xl transition-all duration-300 before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-linear-to-r before:from-transparent before:via-emerald-300/70 before:to-transparent hover:-translate-y-0.5 hover:shadow-[0_28px_90px_-46px_rgba(11,67,54,0.75)]">
      <CardHeader className="border-b border-border/70 pb-4">
        ...
        <CardDescription>
          {isSample
            ? 'Clearly labeled fallback · no call placed'
            : 'Structured patient-reported context · Draft for review'}
        </CardDescription>
        <CardAction>
          <Badge
            variant="outline"
      ...
      <CardContent className="flex min-h-[585px] flex-col">
        {!record ? (
          <div className="my-auto flex flex-col items-center px-6 py-12 text-center">
            <span className="mb-5 grid size-16 place-items-center rounded-2xl border border-emerald-100 bg-emerald-50 text-emerald-700">
            ...
            <p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">
              When the Guava call ends, structured answers appear here
              automatically. Missing values remain explicitly unknown.
            </p>
            <Button
              variant="link"
              size="sm"
              ...
              onClick={onPreview}
            >
              Preview a clearly labeled sample record
            </Button>
          </div>
        ) : (
          <div className="animate-in space-y-4 fade-in slide-in-from-bottom-2 duration-500">
            <div className="flex items-start justify-between gap-4 rounded-2xl bg-[#112a24] p-5 text-white">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.13em] text-emerald-200">
                  {isSample
                    ? 'Demo sample · no call placed'
                    : 'Live Guava check-in'}
                </p>
                <p className="mt-2 text-xl font-semibold">
                  Distress {shown(record.answers.peak_intensity)}/10{' '}
                  <ArrowRight className="mx-1 inline size-4" />{' '}
                  {shown(record.answers.current_intensity)}/10
                </p>
                <p className="mt-1 text-sm text-emerald-100/70">
                  Peak to current self-reported intensity
                </p>
              </div>
              <Badge className="bg-white text-[#112a24]">
                {record.completeness.answered}/{record.completeness.total}{' '}
                captured
              </Badge>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <RecordField
                ...
                value={record.answers.trigger_context}
              />
              <RecordField
                label="Reported symptoms"
                value={record.answers.symptoms}
              />
              <RecordField
                label="What helped"
                value={record.answers.what_helped}
              />
              <RecordField
                label="Clinician follow-up"
                value={
                  record.answers.follow_up_requested === 'yes'
                    ? 'Requested'
                    : record.answers.follow_up_requested === 'no'
                      ? 'Not requested'
                      : null
                }
                highlight={record.answers.follow_up_requested === 'yes'}
              />
            </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-sm">
                <div>
                  ...
                  <p className="mt-0.5 font-semibold">
                    {record.biometrics.max_hr} BPM
                  </p>
                </div>
                <div>
                  <p className="text-muted-foreground">Min HRV</p>
                  <p className="mt-0.5 font-semibold">
                    {record.biometrics.min_hrv} ms
                  </p>
                </div>
                <div>
                  <p className="text-muted-foreground">Recovery</p>
                  <p className="mt-0.5 font-semibold">
                    {formatDuration(record.biometrics.recovery_seconds)}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <ShieldCheck className="size-3.5" /> Clinician review required ·
                Not diagnostic
              </p>
        ...
        <div className="mt-auto rounded-xl border border-dashed border-border bg-muted/40 p-4">
          ...
          <p className="mt-1 text-sm text-foreground/75">
            Wellness signal only · Not a diagnosis · Human clinician remains in
            control
          </p>
        </div>
      </CardContent>
...
function Metric({
  ...
  return (
    <div
      className={`rounded-xl border p-3 transition-colors ${emphasis ? 'border-red-200 bg-red-50/70' : 'border-border/70 bg-card'}`}
    >
      <div className="mb-2 flex items-center gap-1.5 text-xs text-muted-foreground">
        <span className="[&_svg]:size-3.5">{icon}</span>
        {label}
      </div>
      <p className="flex items-baseline gap-1.5 text-2xl font-semibold tracking-[-0.04em]">
        {value}
...
function RecordField({
  ...
  return (
    <div
      className={`rounded-xl border p-4 ${highlight ? 'border-amber-200 bg-amber-50' : 'border-border/70 bg-card'}`}
    >
      <p className="text-xs text-muted-foreground">{label}</p>
      <p
        className={`mt-1.5 text-sm font-medium leading-5 ${!value ? 'italic text-muted-foreground' : ''}`}
      >
        {value || 'Not provided'}
      </p>
    </div>
